...
public class EpcgEnquiryFormDocumentSequenceUpdate extends EntityPersistenceEventObserver {

	private static Entity entities[] = { ModelProvider.getInstance().getEntity(EpcgCostenquiry.ENTITY_NAME) };
	protected Logger logger = Logger.getLogger(EpcgCostenquiry.ENTITY_NAME);
	...
	public void onSave(@Observes EntityNewEvent event) {
		...
		final EpcgCostenquiry issue = (EpcgCostenquiry)event.getTargetInstance();
		
		// update sequence next number
		DocumentType doctype = issue.getDocumentType();
		Sequence seq=doctype.getDocumentSequence();
		Date gidate=issue.getEnquirydate();
		
		String status="";
		
		Date aDate=issue.getEnquirydate();
		...
		if(issue.getAgent()!=null){
			...
			if(issue.getAgent().getRcobCommission().compareTo(issue.getSalescommission())>=0){
	...
	public void onUpdate(@Observes EntityUpdateEvent event) {
		...
		final EpcgCostenquiry issue = (EpcgCostenquiry)event.getTargetInstance();
		if(issue.getAgent()!=null){
			if(issue.getSalescommission().compareTo(issue.getAgent().getRcobCommission())<=0){
				System.out.println("In if con ");
			}else{
				throwError();
			}
	...
	private void throwError() throws OBException {
		String language = OBContext.getOBContext().getLanguage().getLanguage();
		ConnectionProvider conn = new DalConnectionProvider(false);
		throw new OBException(Utility.messageBD(conn, "Agent Commission should not be greater than Default Agennt Commission ", language));
	} 
