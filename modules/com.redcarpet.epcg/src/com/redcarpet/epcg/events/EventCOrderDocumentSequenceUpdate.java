...
public class EventCOrderDocumentSequenceUpdate extends EntityPersistenceEventObserver {

	private static Entity entities[] = { ModelProvider.getInstance().getEntity(Order.ENTITY_NAME) };
	protected Logger logger = Logger.getLogger(Order.ENTITY_NAME);
	...
	public void onSave(@Observes EntityNewEvent event) {
		...
		final Order issue = (Order)event.getTargetInstance();
		
		//update sequence next number
		
		DocumentType doctype = issue.getTransactionDocument();
		Sequence seq=doctype.getDocumentSequence();
		
		Date gidate=issue.getOrderDate();
		
		String status="";
		
		Date aDate=issue.getOrderDate();
