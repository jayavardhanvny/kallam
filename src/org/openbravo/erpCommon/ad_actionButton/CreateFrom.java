                    ...
                    for (int i = 0; i < data.length; i++) {
                        ...
                        try {
                            ...
                            CreateFromInvoiceData.insert(conn, this, strSequence, strKey, vars.getClient(),
                                    ...
                                    data[i].isorder, data[i].epcgPackagingId, data[i].emEpcgNoofpackages);
                            if (strType.equals("SHIPMENT") && isSOTrx.equals("Y")) {
                                CreateFromShipmentToInvoiceData[] shipmentData = CreateFromShipmentToInvoiceData.selectData(new DalConnectionProvider(), data[i].mInoutlineId);
								if (shipmentData != null) {
                                    for (int s = 0; s < shipmentData.length; s++) {
                                String strRateType = shipmentData[s].emRcfrRatetype;
                                String strInsurance = shipmentData[s].emRcfrInsuranceh;
                                String strFreight = shipmentData[s].emRcfrFreighth;
                                String strFreightPaid = shipmentData[s].emRcfrFreightpaid;
                                String strNetunitprice = shipmentData[s].emRcfrPriceactual;
                                String strNetunitrate = shipmentData[s].emRcfrNetunitrate;
                                String strBusinessAgent = shipmentData[s].emEpcgBusinessagent;
                                String strPoreference = shipmentData[s].poreference;
                                String strEpcgType = shipmentData[s].emEpcgType;
                                String strPaymentterm = shipmentData[s].cPaymenttermId;
                                String strPaymentmethod = shipmentData[s].finPaymentmethodId;
                                
                                String qtyOrder = data[i].id;
                                String commission = shipmentData[s].emRcobCommission;
                                strNetunitrate = StringUtils.equals("", strNetunitrate) ? "0" : strNetunitrate;
                                qtyOrder = StringUtils.equals("", qtyOrder) ? "0" : qtyOrder;
                                BigDecimal netRate = new BigDecimal(qtyOrder.replace(",", "")).multiply(new BigDecimal(strNetunitrate));
                                CreateFromShipmentToInvoiceData.updateInvoice(conn, this, shipmentData[s].emRcfrRatetype, shipmentData[s].emRcfrFreighth, 
                                		shipmentData[s].emRcfrFreightpaid, shipmentData[s].emRcfrInsuranceh, shipmentData[s].emEpcgBusinessagent, 
                                		shipmentData[s].emRcobCommission,
                                		shipmentData[s].poreference, shipmentData[s].emEpcgType, shipmentData[s].cPaymenttermId, shipmentData[s].finPaymentmethodId, strKey);
                                CreateFromShipmentToInvoiceData.updateInvoiceLine(conn, this, shipmentData[s].emRcfrPriceactual, shipmentData[s].emRcfrNetunitrate, netRate.toString(), strSequence);
                                CreateFromOrderToInvoiceData.updateAgentLines(conn, this, strKey, data[i].cOrderlineId);
                                    }
                            	}
                            } else if (isSOTrx.equals("Y")) {
                                CreateFromOrderToInvoiceData.updateInvoice(conn, this, data[i].emEpcgBusinessagent, data[i].emEpcgRatetype, data[i].emRcobCommission, strKey);
                                CreateFromOrderToInvoiceData.updateAgentLines(conn, this, strKey, data[i].cOrderlineId);
                            }
                        ...
                        if ("Y".equals(strbreakdown)) {
                            ...
                            while (qty.compareTo(ZERO) > 0) {
                                ...
                                try {
                                    CreateFromShipmentData.insert(conn, this, strSequence, strKey, vars.getClient(),
                                            ...
                                            data[i].explode, data[i].isorder, null, "0", "0", "0");
                            ...
                            try {
                                CreateFromShipmentData.insert(conn, this, strSequence, strKey, vars.getClient(),
                                        ...
                                        data[i].user2Id, data[i].cBpartnerId, data[i].explode, data[i].isorder, null, "0", "0", "0");
            ...
            for (int k = 0; k < ids.length; k++) {
                if (strType.equals("INVOICE")) {
                    ...
                    if (isSOTrx.equals("Y")) {
                        data = CreateFromShipmentData.selectFromPOUpdateSOTrx(conn, this, vars.getLanguage(),
                                ids[k]);
                        CreateFromShipmentData.updateShipmentRateType(conn, this, data[0].ratetype, strKey);
                        ...
                        if (data[i].breakdown.equals("Y")) {
                            ...
                            while (qty.compareTo(ZERO) > 0) {
                                ...
                                try {
                                    CreateFromShipmentData.insert(conn, this, strSequence, strKey, vars.getClient(),
                                            ...
                                            data[i].cCostcenterId, data[i].user1Id, data[i].user2Id, data[i].cBpartnerId,
                                            data[i].explode, data[i].isorder, data[i].epcgPackagingId,
                                            data[i].emEpcgNoofpackages, data[i].netunitrate, data[i].priceactual);
                                    
                                    String strComission = data[i].emRcobCommission;
                                    strComission = StringUtils.equals("", strComission) ? "0" : strComission;
                                    CreateFromShipmentData.updateShipmentCommission(conn, this, strComission, strKey);
                                    if (strType.equals("INVOICE") && !data[i].cInvoicelineId.isEmpty()) {
                                        CreateFromShipmentData.insertInvoiceAcctDimension(conn, this, strSequence,
                                                vars.getClient(), data[i].adOrgId, vars.getUser(), data[i].cInvoicelineId);
                                    } else if (!data[i].cOrderlineId.isEmpty()) {
                            ...
                            try {
                                CreateFromShipmentData.insert(conn, this, strSequence, strKey, vars.getClient(),
                                        ...
                                        data[i].epcgPackagingId, data[i].emEpcgNoofpackages, data[i].netunitrate, data[i].priceactual);

//                                updateInsertedLines(strSequence, data[i].cOrderlineId);
                                    String strComission = data[i].emRcobCommission;
                                    strComission = StringUtils.equals("", strComission) ? "0" : strComission;
                                    CreateFromShipmentData.updateShipmentCommission(conn, this, strComission, strKey);
